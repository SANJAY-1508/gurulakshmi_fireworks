...
if (isset($obj->search_text)) {
    $sql = "SELECT company_id, company_name, mobile_number, gst_no, address FROM company WHERE delete_at = '0'";
...
} else if (!empty($obj->company_id) && !empty($obj->company_name) && !empty($obj->mobile_number) && !empty($obj->gst_no) && !empty($obj->address)) {
    // Required fields
    ...
    // Update query
    $sql = "UPDATE company SET company_name=?, mobile_number=?, gst_no=?, address=? WHERE company_id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssss", $company_name, $mobile_number, $gst_no, $address, $company_id);
